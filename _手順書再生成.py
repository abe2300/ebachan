...
add_h2("6-1. 運用パターン早見表")
add_p("定例報告の運用には4つのパターンがあります。自分に合った方法を選んでください。")
add_table(
    ["パターン", "操作ステップ", "必要なBAT", "自動化度"],
    [
        ["A：手動運用（標準）", "4ステップ：Claude起動→「定例報告」入力→更新確認→bat実行", "GitHubに更新.bat", "△ 基本"],
        ["B：朝の自動更新bat", "2ステップ：bat起動→「定例報告」入力→Enter", "朝の自動更新.bat", "○ 楽"],
        ["C：タスクスケジューラ", "2ステップ：（自動起動）→「定例報告」入力→Enter", "タスクスケジューラ_設定.bat（初回のみ）", "◎ 半自動"],
        ["D：/schedule（クラウド）", "0ステップ：完全自動（現在利用不可）", "なし", "◎◎ 完全自動"],
    ],
)
add_note("💡 「BATを起動するだけ」で完結はしません。ニュース内容の生成はClaudeにしかできないため、必ず「定例報告」と入力する1ステップは残ります（パターンDのみ例外）。")

add_h2("6-2. パターンA：手動運用（標準）の手順")
add_p("最もシンプルな運用方法です。普段はこの方法で十分です。")
add_number("Claude Code を起動する（ショートカットまたは「claude」コマンド）")
add_number("「定例報告」と入力 → Enter")
add_number("Claudeが最新の経済ニュース5件＋株価影響コメントを生成して index.html を直接更新")
add_number("更新内容を確認したら「GitHubに更新.bat」をダブルクリック")
add_number("黒い画面に「Push成功!」が表示されれば完了")
add_number("約1分以内に https://abe2300.github.io/ebachan/ に反映")
add_note("所要時間：1～2分。スマホ・他人もURLでアクセスすれば最新版を閲覧できます。")

add_h3("補足：個別ニュースの差し替え")
add_p("日次更新後、特定のニュース項目だけを差し替えたい場合（例：昼に重要ニュース発表→朝の5件のうち1つを差し替え）：")
add_bullet("Claudeに「○○のニュースに差し替えて」「3番目を○○に変更」などと依頼")
add_bullet("Claudeが該当ニュースだけ書き換え（他は維持）")
add_bullet("更新後、「GitHubに更新.bat」をダブルクリックでpush公開")
add_note("💡 全件を新しい内容で書き換えたい場合は「定例報告」と入力し直してください。")

add_h2("6-3. パターンB：朝の自動更新.bat の手順")
add_p("BATをダブルクリックするだけで、Claude Code起動からpushまで半自動化されます。")

add_h3("起動手順")
add_number("「朝の自動更新.bat」をダブルクリック（デスクトップにショートカットを置くと便利）")
add_number("デスクトップに通知「定例報告アプリ - 朝の自動更新時間です」が表示される")
add_number("Claude Code が自動起動（ローカル版またはブラウザ版）")
add_number("黒い画面に「作業完了後、Enterキーを押してください」と表示される")
add_number("Claude Code 側で「定例報告」と入力 → Enter")
add_number("Claudeが index.html を最新ニュースで更新（完了まで待つ）")
add_number("黒い画面に戻って Enter キーを押す")
add_number("自動で git add → commit → push が実行される")
add_number("「Push成功!」が表示されれば完了 → 約1分後に公開URLに反映")
add_note("⚠️ Claudeの作業が終わるまで黒い画面でEnterを押さないでください。更新前の状態でpushしてしまいます。")

add_h3("デスクトップにショートカットを作る")
add_number("「朝の自動更新.bat」を右クリック")
add_number("「ショートカットの作成」を選択")
add_number("作成されたショートカットをデスクトップに移動")
add_number("毎朝デスクトップから1クリックで起動可能")

add_h2("6-4. パターンC：タスクスケジューラ自動起動の手順")
add_p("毎朝7時に「朝の自動更新.bat」を自動起動させます。PCが起動中である必要があります。詳細セットアップは第7章を参照。")

add_h3("初回セットアップ（1回だけ）")
add_number("「タスクスケジューラ_設定.bat」をダブルクリック")
add_number("確認メッセージで「Y」を押す")
add_number("「設定完了!」と表示されたら登録完了")

add_h3("毎朝の動作（自動）")
add_number("【7:00】タスクスケジューラが「朝の自動更新.bat」を自動起動")
add_number("【7:00】通知が表示され、Claude Codeが自動起動")
add_number("【あなた】「定例報告」と入力 → Enter")
add_number("【あなた】Claude更新完了後、黒い画面でEnter")
add_number("【自動】git push → GitHub Pages公開")
add_note("💡 PCが起動中でないと動きません。スリープ復帰や夜間シャットダウンに注意。")

add_h2("6-5. パターンD：/schedule（クラウド完全自動・将来）")
add_p("Claude Code の /schedule 機能を使うと、クラウド側で完全自動実行されPC起動不要になります。現在は利用不可ですが、復活したら最強の方法です。")
add_bullet("毎朝7時にクラウドが自動でClaude を起動")
add_bullet("プロンプト（_自動更新の指示プロンプト.md の内容）を自動投入")
add_bullet("Claudeが自動でニュース取得＋index.html更新＋git push")
add_bullet("PC不要・操作不要・完全自動")
add_note("📅 /schedule復活時の移行手順は第7章「7-2」を参照。")
